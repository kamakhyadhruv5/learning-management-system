...
export default function AdminHeader({ setMobileOpen, adminName }) {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const dropdownRef = useRef(null);

  const firstLetter = (adminName || 'Admin').charAt(0).toUpperCase();
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/admin/courses?search=${encodeURIComponent(searchQuery)}`);
    }
  };
    ...
    <header className="h-[88px] px-6 sm:px-10 flex items-center justify-between gap-4 border-b border-[#1D363E]/40 relative z-20">
      <div className="flex items-center gap-3 flex-1 max-w-xl">
        ...
        <form onSubmit={handleSearch} className="relative w-full">
          ...
          <input
            ...
            placeholder="Search for users, courses or anything..."
      ...
      <div className="flex items-center space-x-6">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="w-9 h-9 rounded-full bg-[#143e39] text-[#4DE2BD] border border-[#4DE2BD]/40 font-bold flex items-center justify-center text-sm shadow-sm">
              {firstLetter}
            </div>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-[#0D2229] border border-[#1D363E] rounded-2xl py-2 shadow-2xl z-50">
              <div className="px-4 py-2 border-b border-[#1D363E]">
                ...
                <p className="text-sm font-bold text-white truncate">{adminName || 'Administrator'}</p>
              </div>
              <button
                onClick={() => {
                  setDropdownOpen(false);
